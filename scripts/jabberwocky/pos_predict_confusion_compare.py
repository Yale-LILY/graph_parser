...
def regress(x1, x2, y):
    ...
    x2 = x2[perm]
    y = y[perm]
    x_groups_1 = np.split(x1, k_fold)
    x_groups_2 = np.split(x2, k_fold)
    y_groups = np.split(y, k_fold)
    nb_hold_out = nb_words/k_fold
    for i_th in range(k_fold):
        #x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.20, random_state=seed)
        x_test_1 = x_groups_1[i_th]
        x_test_2 = x_groups_2[i_th]
        y_test = y_groups[i_th]
        x_train_1 = np.vstack(x_groups_1[:i_th] + x_groups_1[i_th+1:])
        x_train_2 = np.vstack(x_groups_2[:i_th] + x_groups_2[i_th+1:])
    ...
    # all parameters not specified are set to their defaults
        ...
        logisticRegr.fit(x_train_1, y_train)
        y_predict_1 = logisticRegr.predict(x_test_1)
        logisticRegr = LogisticRegression()
        logisticRegr.fit(x_train_2, y_train)
        y_predict_2 = logisticRegr.predict(x_test_2)
        confusion += confusion_matrix(y_predict_1, y_predict_2)
...
if __name__ == '__main__':
    ...
    path_to_word_embeddings_1 = sys.argv[1]
    path_to_word_embeddings_2 = sys.argv[2]
    path_to_word_embeddings_vocab = 'scripts/jabberwocky/word_counts.txt' 
    new_embeddings_1, idxes = get_data(path_to_vocab, path_to_pos_y, path_to_word_embeddings_1, path_to_word_embeddings_vocab)
    new_embeddings_2, idxes = get_data(path_to_vocab, path_to_pos_y, path_to_word_embeddings_2, path_to_word_embeddings_vocab)
    confusion = regress(new_embeddings_1, new_embeddings_2, idxes)
