...
from utils.equations.gru import get_gru_weights, gru
from utils.equations.srnn import get_srnn_weights, srnn
...
class Basic_Model(object):
    ...
    def add_gru(self, inputs, i, name, backward=False):
        prev_init = tf.zeros([tf.shape(inputs)[1], self.opts.units])  # [batch_size, num_units]
        #prev_init = tf.zeros([2, 100, self.opts.units])  # [2, batch_size, num_units]
        if i == 0:
            inputs_dim = self.inputs_dim
        else:
            inputs_dim = self.opts.units*2 ## concat after each layer
        weights = get_gru_weights('{}_GRU_layer{}'.format(name, i), inputs_dim, self.opts.units, tf.shape(inputs)[1], self.hidden_prob)
        if i == 0:
            self.input_weights = weights
        if backward:
            ## backward: reset states after zero paddings
            non_paddings = tf.transpose(self.weight, [1, 0]) ## [batch_size, seq_len] => [seq_len, batch_size]
            non_paddings = tf.reverse(non_paddings, [0])
            cell_hidden = tf.scan(lambda prev, x: gru(prev, x, weights, backward=backward), [inputs, non_paddings], prev_init)
        else:
            cell_hidden = tf.scan(lambda prev, x: gru(prev, x, weights), inputs, prev_init)
         #cell_hidden [seq_len, 2, batch_size, units]
        #h = tf.unstack(cell_hidden, 2, axis=1)[1] #[seq_len, batch_size, units]
        h = cell_hidden
        return h

    def add_srnn(self, inputs, i, name, backward=False):
        prev_init = tf.zeros([tf.shape(inputs)[1], self.opts.units])  # [batch_size, num_units]
        #prev_init = tf.zeros([2, 100, self.opts.units])  # [2, batch_size, num_units]
        if i == 0:
            inputs_dim = self.inputs_dim
            ...
            inputs_dim = self.opts.units*2 ## concat after each layer
        weights = get_srnn_weights('{}_SRNN_layer{}'.format(name, i), inputs_dim, self.opts.units, tf.shape(inputs)[1], self.hidden_prob)
        if i == 0:
            self.input_weights = weights
        if backward:
            ## backward: reset states after zero paddings
            non_paddings = tf.transpose(self.weight, [1, 0]) ## [batch_size, seq_len] => [seq_len, batch_size]
            non_paddings = tf.reverse(non_paddings, [0])
            cell_hidden = tf.scan(lambda prev, x: srnn(prev, x, weights, backward=backward), [inputs, non_paddings], prev_init)
        else:
            cell_hidden = tf.scan(lambda prev, x: srnn(prev, x, weights), inputs, prev_init)
         #cell_hidden [seq_len, 2, batch_size, units]
        #h = tf.unstack(cell_hidden, 2, axis=1)[1] #[seq_len, batch_size, units]
        h = cell_hidden
        return h
